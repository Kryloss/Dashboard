-- SIMPLIFIED FIX FOR WORKOUT DATA ISOLATION
-- This addresses the cross-user data leakage issue
-- Run this SQL in your Supabase SQL Editor

-- ============================================================================
-- STEP 1: MODIFY SCHEMA TO ALLOW NULL USER_ID FOR BUILT-IN TEMPLATES
-- ============================================================================

-- First, we need to modify the foreign key constraint to allow NULL user_id for built-in templates
-- This way built-in templates won't be tied to any specific user

ALTER TABLE public.workout_templates 
DROP CONSTRAINT IF EXISTS workout_templates_user_id_fkey;

-- Add the constraint back, but allow NULL for built-in templates
ALTER TABLE public.workout_templates 
ADD CONSTRAINT workout_templates_user_id_fkey 
FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;

-- Allow user_id to be NULL for built-in templates
ALTER TABLE public.workout_templates 
ALTER COLUMN user_id DROP NOT NULL;

-- ============================================================================
-- STEP 2: CREATE SYSTEM-WIDE BUILT-IN TEMPLATES (NO USER OWNER)
-- ============================================================================

-- Clean up any existing per-user built-in templates first
DELETE FROM public.workout_templates WHERE is_built_in = true;

-- Insert system built-in templates with NULL user_id (system-wide)
INSERT INTO public.workout_templates (id, user_id, name, type, exercises, is_built_in, created_at, updated_at)
VALUES 
(
    '11111111-0000-0000-0000-000000000001'::UUID,
    NULL, -- NULL user_id means system-wide
    'Push Day',
    'strength',
    '[
        {
...
-- ============================================================================
-- STEP 3: FIX RLS POLICIES WITH EXPLICIT AUTH CHECKS
-- ============================================================================
...
CREATE POLICY "Users can view own templates and system built-ins ONLY" ON workout_templates
    FOR SELECT USING (
        auth.uid() IS NOT NULL AND 
        (
            -- User's own templates (must have matching user_id)
            (auth.uid() = user_id AND user_id IS NOT NULL) OR
            -- System built-in templates (user_id is NULL and is_built_in is true)
            (user_id IS NULL AND is_built_in = true)
        )
    );
...
-- Count system built-in templates (should be exactly 3 with NULL user_id)
SELECT 
    COUNT(*) as system_builtin_count,
    COUNT(user_id) as templates_with_user_id,
    COUNT(*) - COUNT(user_id) as templates_without_user_id
...
BEGIN 
    ...
    RAISE NOTICE '🌍 Built-in templates are now system-wide (user_id = NULL)';
    RAISE NOTICE '🛡️ All database operations now require proper authentication';
    RAISE NOTICE '🧹 Cleaned up per-user built-in templates';
    RAISE NOTICE '';
    RAISE NOTICE 'Next steps:';
    RAISE NOTICE '1. Test with different user accounts';
    RAISE NOTICE '2. Verify each user only sees their own workouts + 3 system built-ins';
